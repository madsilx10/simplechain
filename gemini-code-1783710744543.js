...
// User-Agent disamakan agar Cloudflare tidak curiga
...
async function solveTurnstile() {
  // Browser tetap jalan di latar belakang (headless: true)
  ...
  const context = await browser.newContext({
    ...
    viewport: { width: 400, height: 400 } // Kecil biar hemat RAM Termux
  });
  const page = await context.newPage();

  try {
    // Blokir request gambar/font biar super ringan & cepet
    ...
    log('Membuka halaman faucet asli (Menembus Cloudflare)...');
    
    // Langsung buka ke web asli supaya Turnstile tidak error "Unable to connect"
    ...
    // Polling token langsung dari struktur halaman asli
    let tokenValue = '';
    for (let i = 0; i < 45; i++) {
    ...
    if (!tokenValue) {
      ...
      throw new Error('Turnstile token timeout bray. Cek screenshot_failed.png');
    }
    
    return tokenValue;
  } finally {
    // Pastikan browser ditutup biar gak ngebegal RAM HP lu
    await browser.close();
  }
}
...
async function claimFaucet(wallet) {
  ...
  // Nembak API dengan User-Agent yang sinkron dengan browser
...
async function main() {
  if (!fs.existsSync(WALLETS_FILE)) {
    err(`File ${WALLETS_FILE} kagak ada bray! Bikin dulu file-nya terus isi wallet lu.`);
    process.exit(1);
  }
