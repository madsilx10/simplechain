...
async function claimFaucet(address) {
  ...
  try {
    ...
    const context = await browser.newContext({
      ...
      viewport: { width: 1280, height: 1000 }, // Ditinggiin biar keliatan semua tombolnya
      locale: 'en-US'
    });
    ...
    // Tembak placeholder yang sesuai gambar "Enter your SimpleChain2.0..."
    ...
    // --- PROSES CLICK SUBMIT (FIXED BERDASARKAN GAMBAR) ---
    console.log(`[${address}] Mencari tombol "Send 0.1 SRW"...`);
    
    // Tembak text spesifik yang ada di tombol toska panjang pada gambar lo
    const btn = page.locator('text="Send 0.1 SRW", :has-text("Send 0.1 SRW")').first();
    await btn.waitFor({ state: 'visible', timeout: 15000 });
    
    console.log(`[${address}] Klik submit...`);
    // Gunakan force true biar eksekusinya instan tanpa ketahan bug scroll browserless
