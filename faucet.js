...
async function claimFaucet(address) {
  ...
  try {
    ...
    // Tunggu element utama ke-render
    ...
    // --- STRATEGI KRITIKAL: TUNGGU TOKEN TURNSTILE SELESAI GENERATE ---
    console.log(`[${address}] Menunggu Cloudflare mengeluarkan token sukses...`);
    try {
      // Cloudflare menaruh token sukses di input tersembunyi bernama cf-turnstile-response
      ...
      console.log(`[${address}] ⚠️ Peringatan: Menunggu token timeout, lanjut coba input...`);
    }
    ...
    // Menunggu respons dari API (looping max 20 detik karena cloud butuh waktu transmit)
    ...
    if (capturedResponse) {
      ...
      console.log(`[${address}] ⚠️ Gak dapet respons API. Kemungkinan besar IP Browserless atau Wallet lo sedang cooldown (Limit 24 Jam).`);
    }
